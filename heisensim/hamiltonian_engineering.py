...
class WAHUHA:
    def __init__(self, h, taus, phase=0, min_iteration=1, max_time=None, final_relaxation=None):
        self.h = h
        self.taus = taus
        self.phase = phase
    ...
    def cycle_fields(self):
        ...
        h = self.h
        return [
            (0, 0, 0), self.get_pulse(h, 0 + phase),
            (0, 0, 0), self.get_pulse(h, -np.pi/2 + phase),
            (0, 0, 0), self.get_pulse(-h, -np.pi/2 + phase),
            (0, 0, 0), self.get_pulse(-h, 0 + phase),
            (0, 0, 0)
        ]
...
class WAHUHA_amp_errors(WAHUHA):
    def __init__(self, h, taus, phase=0, min_iteration=1, max_time=None, phase_error=0, final_relaxation=None):
        super().__init__(h, taus, phase=phase, min_iteration=min_iteration, max_time=max_time, final_relaxation=final_relaxation)
        self.phase_error = phase_error
        if phase != 0:
            raise ValueError("phase needs to be zero. Otherwise, use standard WAHUHA sequence.")
